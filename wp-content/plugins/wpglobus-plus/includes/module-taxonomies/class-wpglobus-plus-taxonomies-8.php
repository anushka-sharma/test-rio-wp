	...
	class WPGlobusPlus_Taxonomies {
		...
		public function __construct( $wpg_plus ) {
			...
			if ( is_admin() ) {
				 ...
				 * @since 1.3.4 @W.I.P
				 * 
				 * may be to use get_terms() in wp-includes\class-wp-term-query.php
				 */
				//add_filter( 'get_object_terms', array( $this, 'filter__get_object_terms' ), 10, 4 ); 
				
				/**
				 * @see `get_the_terms` in wp-includes\category-template.php
				 * @since 1.3.4
				 */
				add_filter( 'get_the_terms', array( $this, 'filter__get_the_terms' ), 10, 3 );

				/**
				 * @see `get_term_link()` in wp-includes\taxonomy.php
				 * @since 1.3.4
				 */
				add_filter( 'term_link', array( $this, 'filter__term_link' ), 10, 3 );				
